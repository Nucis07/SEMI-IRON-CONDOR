...
stacca_dd = dd_pct <= -DD_STACCA_CALL
...
if gap>15 and shock_recente:
    ...
    lente=("🟠","vol in normalizzazione (realizzata ancora alta): finestra di premio, "
                "ma e' la zona del rimbalzo violento sulla call: se il drawdown e' profondo la regola stacca-call ti copre.")
else:
...
if stacca_call:
    ...
    tab=pd.DataFrame({
        "gamba":["VENDI CALL","VENDI PUT","COMPRA LONG PUT"],
...
pronto = (mp>0 and ml>0 and ala_op>0 and (stacca_call or mc>0))
if pronto:
    # equo arrotondati come finiranno nel foglio (per far combaciare i numeri)
    fcr,fpr,flr=round(f_c),round(f_p),round(f_l)
    # sensore BANDA: markup 3 gambe misurato sul book (solo se la call e' quotata)
    ne3_ = fcr+fpr-flr
    mk3_ = (mc+mp-ml)/ne3_ if (mc>0 and ne3_>0) else float("nan")
    stacca_band = (MK_BANDA is not None and not stacca_dd and mk3_==mk3_
                   and MK_BANDA[0] <= mk3_ < MK_BANDA[1])
    ...
    net_equo=(0 if stacca_call else fcr)+fpr-flr   # equo AGLI STRIKE OPERATI, struttura effettiva
    ...
    c_ = (lambda x: "" if stacca_call else x)   # call vuota se staccata
    # eseguito per gamba: valore solo se l'utente l'ha messo (ce/pe/le > 0)
    exe_c = "" if stacca_call else (f"{ce:.0f}" if ce>0 else "")
    exe_p = f"{pe:.0f}" if pe>0 else ""
    exe_l = f"{le:.0f}" if le>0 else ""
    n_exe = sum(1 for x in ([exe_c] if not stacca_call else []) + [exe_p, exe_l] if x != "")
    n_att = (2 if stacca_call else 3)
    esito_fill = "completo" if n_exe==n_att else ("parziale" if n_exe>0 else "non operato")
    riga=[oggi, f"{P0:.0f}", c_(f"{Kcall_op:.0f}"), f"{Kps_op:.0f}", f"{Kpw_op:.0f}",
          exe_c, exe_p, exe_l,
          c_(f"{cb:.0f}"), c_(f"{ca:.0f}"), f"{pb:.0f}", f"{pa:.0f}", f"{lb:.0f}", f"{la:.0f}",
          c_(f"{mc:.0f}"), f"{mp:.0f}", f"{ml:.0f}",
